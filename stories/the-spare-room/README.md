# The Spare Room

*A darkly funny family play about a woman who once made room for her mother inside herself—and now wants her to leave.*

## Status

**Complete second draft. Blocked workshop pending.**

The continuous manuscript is in [chapters/](chapters/README.md), rebuilt as seven rehearsal movements. The divisions do not imply blackouts or scenic changes. The current [English Markdown reading copy](../../compiled/the-spare-room.md) is compiled from those sources.

A [stage curator's review](editorial/CURATOR_REVIEW_2026-09-22.md) prompted this rework of the full first draft. The [response and change record](editorial/RESPONSE_TO_CURATOR_2026-09-22.md) explains the decisions. The previous text and its development notes are preserved in the [first-draft baseline](drafts/first-draft-2026-09-22/README.md).

Approximately seventy minutes remains an ambition, not a measured runtime. The next step is the [blocked workshop](WORKSHOP_GUIDE.md): three actors, actual ordinary objects and spectators unfamiliar with the script. No workshop or audience evidence has been obtained.

A separate [45-minute audio adaptation](adaptations/audio/README.md) was commissioned on 23 September 2026. Its [work order](adaptations/audio/WORK_ORDER.md) calls for separate character voices, sparse narration and a performed listening test. It has its own manuscript and development record; drafting is pending. The stage sources and the stage workshop remain distinct from this adaptation.

PDF generation is left to the existing CI at the author's request. The current local PDF may still contain the first draft until CI runs; use the linked Markdown for this revision.

## Form

- Stage play, approximately seventy minutes without an interval.
- Three performers: Ruth, Helen and Kit.
- One stable playing space, ordinary furniture and clothes.
- No additional voices, projections, effects or scenic transformations.

The audience first meets a family arguing about a move. As the room's nature becomes clearer, it enables actions that would be impossible in an ordinary home. The immediate wants remain recognisable.

## The argument

Helen wants Ruth to move out today. Kit wants Helen at the beginning of their new household tonight, and has exaggerated both women's agreement to get things moving.

Ruth discovers she wants the new life on offer. Helen then discovers she has expected to retain access to the familiar mother inside it.

> Can you love your mother and still want her out of your head?

A second pressure develops through the action: how much of the person you recognise can you ask someone to keep being?

## Current artifacts

The Swedish translation, **Gästrummet**, and existing audio remain **first-draft materials**. They are preserved in [translations/svenska/](translations/svenska/kapitel/README.md), with the [translation plan](translations/svenska/TRANSLATION_PLAN.md). They have not been revised to match this English second draft. The Swedish names are Rut, Helena and Kim.

| File | Purpose |
| --- | --- |
| [chapters/](chapters/README.md) | Complete second draft in seven continuous movements |
| [PROJECT_BRIEF.md](PROJECT_BRIEF.md) | Source commission and production constraints |
| [OUTLINE.md](OUTLINE.md) | Seven movements, consequential turns and revelation order |
| [EXAMPLE_DRAFT.md](EXAMPLE_DRAFT.md) | Revision 3 source passage retained for development history |
| [CHARACTERS.md](CHARACTERS.md) | Current particular lives, desires and compromises |
| [CONCEPTUAL_MAP.md](CONCEPTUAL_MAP.md) | Private continuity, room rules and the speculative action |
| [TONE_AND_VOICE.md](TONE_AND_VOICE.md) | Voice range and performance questions |
| [DEVELOPMENT_DIARY.md](DEVELOPMENT_DIARY.md) | Conversation history and reasons for revisions |
| [PROGRESS_SUMMARY.md](PROGRESS_SUMMARY.md) | Completed work, unresolved risks and evidence still needed |
| [WORKSHOP_GUIDE.md](WORKSHOP_GUIDE.md) | Blocking, unbriefed audience questions and evidence to collect |
| [adaptations/audio/](adaptations/audio/README.md) | Commission for a 45-minute English audio adaptation; script and listening test pending |

## Review and earlier versions

- [Curator review](editorial/CURATOR_REVIEW_2026-09-22.md) and [response](editorial/RESPONSE_TO_CURATOR_2026-09-22.md): second-draft rework and reasons for accepting or qualifying the criticism.
- [First-draft baseline](drafts/first-draft-2026-09-22/README.md): nine movements, original compiled text and PDF, frozen development notes and hash manifest.
- [First-draft review, 2026-09-22](editorial/FIRST_DRAFT_REVIEW_2026-09-22.md): two critique passes, changes made, and table-read questions.
- [Producer follow-up, 2026-09-22](editorial/PRODUCER_FOLLOWUP_2026-09-22.md): two narrow rework requests and three tests for drafting. The reviewer had assessed the response note, not the revised dramatic pages.
- [Outline, revision 2](drafts/outline-v2.md) and [dramatic pages, revision 2](drafts/dialogue-v2.md): preserved before the limited handoff pass.
- [Response to the producer, 2026-09-22](editorial/RESPONSE_TO_PRODUCER_2026-09-22.md): changes made, reasons for accepting the five principal criticisms, qualifications and work still to test.
- [Producer review, received 2026-09-22](editorial/PRODUCER_REVIEW_2026-09-22.md): source text, including commercial suggestions not independently verified.
- [Outline, revision 1](drafts/outline-v1.md).
- [Dialogue samples, revision 1](drafts/dialogue-v1.md): includes the retained swimming-bag opening.

## Commitments

The family conflict must be followable before the audience understands the circumstances. Ruth's personhood is ordinary and settled. Helen is her own self-representation inside her mind, her idea of herself in these family contexts. The door remains an entrance and exit, not a transformation device.

The chair's changing meaning must alter what somebody does. Ruth's move leads to an independent life. Its risks are those of living and changing, not hidden deletion or death.

The people can be petty, funny, inarticulate and unfair. Their desires should outrun their accounts of themselves.

## Working references

The original conversation drew on *Waiting for Godot* for economy, Kaufman for emotionally legible high concept, and *I'm Thinking of Ending Things* for the reclassification of ordinary encounters through memory and projection. It also discussed *The Real Thing*, *Arcadia* and *The Father*.

These remain references for particular formal possibilities, not templates or claims of equivalence. The producer's additional comparisons are preserved in the review. The current task is to find the play's own consequential action.
